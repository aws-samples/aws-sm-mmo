...
class Bertbase(Base):
	def inference(self, text):
		...
		text_len=len(tokens.input_ids[0])
		...
		input_ids = tritonclient.http.InferInput(name="input_ids", shape=(batch_size, text_len), datatype="INT64")
		token_type_ids = tritonclient.http.InferInput(name="token_type_ids", shape=(batch_size, text_len), datatype="INT64")
		attention = tritonclient.http.InferInput(name="attention_mask", shape=(batch_size, text_len), datatype="INT64")
		...
		for token, score in top_5_tokens:
			data = { 'token' : text.replace(tokenizer.mask_token, tokenizer.decode([token])), 'score' : score}
			lista.append(data)
		return lista
